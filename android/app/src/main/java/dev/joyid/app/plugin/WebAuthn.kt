...
class WebAuthn {
    ...
    suspend fun create(call: PluginCall) {
        ...
        try {
            val response = credentialManager.createCredential(request, activity) as CreatePublicKeyCredentialResponse
    ...
    suspend fun get(call: PluginCall) {
        val requestJson: String = call.data.toString()
        val publicKeyCredentialOption = GetPublicKeyCredentialOption(requestJson, null, true);
    ...
    private fun handleCreateCredentialSuccess(
        ...
        try {
            call.resolve(JSObject(response.registrationResponseJson))
